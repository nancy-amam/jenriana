...
export default function ApartmentDetails({ apartment }: { apartment: Apartment }) {
  ...
  return (
    <div className="relative w-full min-h-screen">
      {/* Background Image */}
      <div className="relative w-full h-[841px] mb-6">
        <Image
        ...
        {/* Overlay Content */}
        <div className="absolute bottom-10 left-10 right-10 text-white flex flex-col md:flex-row md:justify-between gap-6">
          <div className="px-10 mt-20 max-w-[60%]">
            <h1 className="text-[48px] mt-5 font-normal">{apartment.name}</h1>
            <p className="text-[20px] mt-2">{apartment.location}</p>
            <div className="flex mt-2 items-center gap-2 text-sm">
              <StarIcon className="w-4 h-4 text-yellow-400" />
              <span className=" text-[#d1d5db]">{apartment.rating} (views)</span>
            </div>
            <div className="text-[30px] mt-2 font-normal">${apartment.price}<span className=" text-sm"> / night</span> </div>
          </div>

          {/* Booking Card */}
          <div className="bg-[#f1f1f1] text-[#1e1e1e] rounded-xl p-6 w-full max-h-[378px] md:max-w-[460px] border border-gray-200 shadow-md">
...
>
            ...
            <div className="px-6 max-w-[1400px] mx-auto mb-10">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="md:col-span-2">
                  <Image
                    src={apartment.galleryImages[0].src}
  ...
  {apartment.amenities.map((amenity) => {
    ...
    return (
      <div
        key={amenity.id}
        className="w-[286px] h-[108px] border border-gray-200 rounded-[12px] p-6 flex flex-col items-center justify-center gap-2 text-center"
      >

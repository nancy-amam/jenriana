...
function BookingEngineContent() {
  ...
  // Transform apartment to ensure compatibility
  ...
  const handleConfirmAndPay = () => {
    const booking = {
      ...
      apartmentId: transformedApartment.id,
      apartmentName: transformedApartment.name,
      apartmentLocation: transformedApartment.location,
      checkInDate,
      checkOutDate,
      nights,
      guests: numGuests,
    ...
    <div className="bg-[#f1f1f1] py-12 px-4 md:px-16">
      <div className="max-w-[1550px] mx-auto grid grid-cols-1 md:grid-cols-[757px_minmax(0,1fr)] md:gap-8">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="bg-white rounded-lg p-6 shadow-md w-full">
            <div className="flex flex-col sm:flex-row sm:justify-between">
              ...
              {transformedApartment.galleryImages && transformedApartment.galleryImages.length > 0 ? (
                <div className="order-1 sm:order-2 mb-8 sm:mb-10 sm:ml-4 overflow-x-auto flex gap-2 sm:block sm:overflow-visible">
                  {transformedApartment.galleryImages.slice(0, 4).map((img) => (
                    <Image
                      key={img.id}
              ...
              <h2 className="order-2 sm:order-1 text-2xl font-normal text-[#111827]">
                {transformedApartment.name}
              </h2>
            </div>

            <p className="flex items-center text-base text-[#4b5566] md:-mt-20">
              <MapPin className="w-4 h-4 mr-1 text-[#4b5566]" />
              {transformedApartment.location}
            </p>

            <p className="text-[30px] font-normal text-[#111827]">
              {transformedApartment.price.toLocaleString("en-NG", { style: "currency", currency: "NGN", maximumFractionDigits: 0 })}
            ...
            <div className="space-y-4 text-[#374151]">
              <div className="grid grid-cols-2 gap-4">
                <div className="flex flex-col">
                  <label htmlFor="check-in-date" className="font-normal text-sm text-[#374151] mb-1">
                    Check-in:
                  </label>
                  <input
                    id="check-in-date"
                ...
                <div className="flex flex-col">
                  <label htmlFor="check-out-date" className="font-normal text-sm text-[#374151] mb-1">
                    Check-out:
                  </label>
                  <input
                    id="check-out-date"
              ...
              <div className="flex flex-col">
                <label htmlFor="guests" className="font-normal text-sm text-[#374151] mb-1">
                  Number of Guests:
                </label>
                <select
                  id="guests"
              ...
              <div className="pt-4 mt-4 space-y-2">
                ...
                <div className="flex justify-between text-sm">
                  <span>
                    {price.toLocaleString("en-NG", { style: "currency", currency: "NGN", maximumFractionDigits: 0 })} x {nights} night(s)
                  </span>
                  <span>{totalCost.toLocaleString("en-NG", { style: "currency", currency: "NGN", maximumFractionDigits: 0 })}</span>
                </div>
                <div className="flex justify-between text-[#4b5566] text-sm">
                  <span>Service Fee</span>
                  <span>{serviceFee.toLocaleString("en-NG", { style: "currency", currency: "NGN", maximumFractionDigits: 0 })}</span>
                </div>
                {initialSelectedServices.map((serviceId) => {
                  ...
                  return service ? (
                    <div key={service.id} className="flex justify-between text-[#4b5566] text-sm">
                      ...
                      <span>{service.price.toLocaleString("en-NG", { style: "currency", currency: "NGN", maximumFractionDigits: 0 })}</span>
                    </div>
                  ) : null;
                })}
                <div className="flex justify-between text-[#4b5566] text-sm">
                  <span>Taxes (7.5%)</span>
                  <span>{taxes.toLocaleString("en-NG", { style: "currency", currency: "NGN", maximumFractionDigits: 0 })}</span>
                </div>
                <div className="flex justify-between font-bold text-lg text-[#111827] border-t border-gray-300 pt-2">
                  <span>Total</span>
                  <span>{grandTotal.toLocaleString("en-NG", { style: "currency", currency: "NGN", maximumFractionDigits: 0 })}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="bg-white rounded-lg p-6 shadow-md w-full md:max-h-[480px] mt-4 md:mt-0">
          ...
          >
            Confirm and Pay {grandTotal.toLocaleString("en-NG", { style: "currency", currency: "NGN", maximumFractionDigits: 0 })}
          </button>
